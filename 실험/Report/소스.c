...
int main()
{
	...
	// 사진을 흑백으로 바꿔주는 함수
	egdt();
	// 위의 bitof24_to_8bit_grapy에서 흑백으로 바꾼 사진을 받아와 edge detecting 하는 함수.
...
void egdt(void)
{
	...
	for (i = 0; i < hinfo.biHeight; i++)
		for (j = 0; j < hinfo.biWidth; j++)
			...
			// 이전에 계산해뒀던 최대값과 최소값 및 BYTE_MAX를 통해 필터링해 얻어낸 변화량을 0부터 255범위로 재설정한다.
